    ...
    if not services:
        ...
        for service in services:
            builder.row(InlineKeyboardButton(
                text=service,
                callback_data=ServiceCallback(action=action_prefix, service_name=service).pack()
            ))
...
def get_countries_keyboard(service_name: str, action_prefix: str) -> InlineKeyboardMarkup:
    ...
    if service_name not in mock_db["services"]:
        ...
        builder.row(InlineKeyboardButton(
            ...
            callback_data=NavCallback(action="back").pack()
        ))
        return builder.as_markup()
    ...
    if not countries:
        ...
        for country in countries:
            builder.row(InlineKeyboardButton(
                text=country,
                callback_data=CountryCallback(action=action_prefix, service_name=service_name, country_name=country).pack()
            ))
...
async def handle_cancel_operation(message: Message, state: FSMContext):
    ...
    if current_state is None:
        await message.answer("আপনি বর্তমানে কোনো অপারেশনে নেই।")
        return
    await state.clear()
    await message.answer("অপারেশন বাতিল করা হয়েছে। প্রধান মেনুতে ফিরে আসা হলো।")
...
async def admin_add_service_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "নতুন সার্ভিসের নাম লিখুন (যেমন: WhatsApp, Telegram):",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Cancel", callback_data="cancel_fsm")]])
    )
...
async def admin_add_service_name_input(message: Message, state: FSMContext):
    ...
    if service_name in mock_db["services"]:
        await message.answer(
            f"'{service_name}' নামে সার্ভিস আগে থেকেই আছে। অন্য নাম দিন:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Cancel", callback_data="cancel_fsm")]])
        )
    else:
        mock_db["services"][service_name] = {}
        await state.clear()
        await message.answer(f"✅ সার্ভিস '{service_name}' সফলভাবে যোগ করা হয়েছে।")
        logging.info(f"Admin added service: {service_name}. DB: {mock_db}")
...
async def admin_add_country_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "কোন সার্ভিসের অধীনে দেশ যোগ করতে চান?",
        reply_markup=get_services_keyboard(action_prefix="select_for_add_country")
    )
...
async def admin_add_country_service_selected(query: CallbackQuery, callback_data: ServiceCallback, state: FSMContext):
    ...
    await query.message.edit_text(
        f"<b>সার্ভিস: {service_name}</b>\n\n"
        "এই সার্ভিসে দেশ যোগ করতে, নিচে দেশের নাম টাইপ করে সেন্ড করুন।",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Back to Services", callback_data=NavCallback(action="back").pack())]
        ])
    )
    await query.answer()
...
async def admin_add_country_name_input(message: Message, state: FSMContext):
    ...
    if country_name in mock_db["services"][service_name]:
        await message.answer(
            f"'{country_name}' দেশটি '{service_name}' সার্ভিসে আগে থেকেই আছে। অন্য নাম দিন:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Back to Services", callback_data=NavCallback(action="back").pack())]])
        )
    else:
        mock_db["services"][service_name][country_name] = []
        await state.clear()
        await message.answer(f"✅ দেশ '{country_name}' সফলভাবে '{service_name}' সার্ভিসে যোগ করা হয়েছে।")
        logging.info(f"Admin added country: {country_name} to {service_name}. DB: {mock_db}")
...
async def admin_add_number_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "কোন সার্ভিসে নম্বর যোগ করতে চান?",
        reply_markup=get_services_keyboard(action_prefix="select_for_add_num")
    )
...
async def admin_add_number_country_selected(query: CallbackQuery, callback_data: CountryCallback, state: FSMContext):
    await state.update_data(service_name=callback_data.service_name, country_name=callback_data.country_name)
    await state.set_state(AdminStates.add_number_method_choice)
    
    method_keyboard = InlineKeyboardBuilder()
    method_keyboard.row(InlineKeyboardButton(text="✍️ Add via Text", callback_data="add_num:text"))
    method_keyboard.row(InlineKeyboardButton(text="📄 Add via Text File", callback_data="add_num:file"))
    method_keyboard.row(InlineKeyboardButton(text="🔙 Back to Countries", callback_data=NavCallback(action="back").pack()))

    await query.message.edit_text(
        f"<b>সার্ভিস: {callback_data.service_name}</b>\n"
        f"<b>দেশ: {callback_data.country_name}</b>\n\n"
...
async def handle_add_num_text_choice(query: CallbackQuery, state: FSMContext):
    await state.set_state(AdminStates.add_number_input_text)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Back to Method Choice", callback_data=NavCallback(action="back").pack())]])
    await query.message.edit_text(
        "<b>নির্দেশনা:</b>\n"
        "অনুগ্রহ করে নম্বরগুলি টাইপ করুন। প্রতিটি নম্বর একটি নতুন লাইনে লিখুন:",
...
async def handle_add_num_file_choice(query: CallbackQuery, state: FSMContext):
    await state.set_state(AdminStates.add_number_input_file)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Back to Method Choice", callback_data=NavCallback(action="back").pack())]])
    await query.message.edit_text(
        "<b>নির্দেশনা:</b>\n"
        "অনুগ্রহ করে একটি <b>.txt</b> ফাইল আপলোড করুন। ফাইলের প্রতিটি নম্বর একটি নতুন লাইনে থাকতে হবে:",
        reply_markup=keyboard
    )
    await query.answer()

async def process_numbers(text_data: str, service_name: str, country_name: str) -> int:
    numbers = [num.strip() for num in text_data.splitlines() if num.strip()]
    count = 0
    if country_name not in mock_db["services"][service_name]:
        mock_db["services"][service_name][country_name] = []
    for num in numbers:
        if num not in mock_db["services"][service_name][country_name]:
            mock_db["services"][service_name][country_name].append(num)
            count += 1
    return count
...
async def admin_add_number_text_input(message: Message, state: FSMContext):
    ...
    if not service or not country or service not in mock_db["services"]:
        await state.clear(); await message.answer("কিছু একটা ভুল হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।"); return
    count = await process_numbers(message.text, service, country)
    await state.clear()
    await message.answer(f"✅ সফলভাবে {count} টি নতুন নম্বর '{country}' ({service}) তে যোগ করা হয়েছে।")
    logging.info(f"Admin added {count} numbers via text.")
...
async def admin_add_number_file_input(message: Message, state: FSMContext):
    if not message.document.mime_type == "text/plain":
        await message.answer("অনুগ্রহ করে একটি .txt ফাইল আপলোড করুন।"); return
    data = await state.get_data(); service = data.get("service_name"); country = data.get("country_name")
    if not service or not country or service not in mock_db["services"]:
        await state.clear(); await message.answer("কিছু একটা ভুল হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।"); return
    try:
        ...
        count = await process_numbers(text_data, service, country)
        await state.clear()
        await message.answer(f"✅ ফাইল থেকে সফলভাবে {count} টি নতুন নম্বর '{country}' ({service}) তে যোগ করা হয়েছে।")
        logging.info(f"Admin added {count} numbers via file.")
    except Exception as e:
        await message.answer(f"ফাইল প্রসেস করতে সমস্যা হয়েছে: {e}")
...
async def admin_remove_service_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "আপনি কোন সার্ভিসটি মুছে ফেলতে চান?",
        reply_markup=get_services_keyboard(action_prefix="remove_service") 
    )
...
async def admin_remove_service_selected(query: CallbackQuery, callback_data: ServiceCallback, state: FSMContext):
    service_name = callback_data.service_name
    try:
        del mock_db["services"][service_name]
        await state.clear(); await query.message.edit_text(f"✅ সার্ভিস '{service_name}' সফলভাবে মুছে ফেলা হয়েছে।")
        logging.info(f"Admin removed service: {service_name}.")
    except KeyError:
        await state.clear(); await query.message.edit_text("❌ ত্রুটি: সার্ভিসটি খুঁজে পাওয়া যায়নি।")
    await query.answer()
...
async def admin_remove_country_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "কোন সার্ভিস থেকে দেশ মুছতে চান?",
        reply_markup=get_services_keyboard(action_prefix="select_for_remove_country")
    )
...
async def admin_remove_country_service_selected(query: CallbackQuery, callback_data: ServiceCallback, state: FSMContext):
    service_name = callback_data.service_name
    await state.update_data(service_name=service_name)
    await state.set_state(AdminStates.remove_country_select)
    await query.message.edit_text(
        f"সার্ভিস: {service_name}\n\nআপনি কোন দেশটি মুছে ফেলতে চান?",
        reply_markup=get_countries_keyboard(service_name, action_prefix="remove_country")
    )
    await query.answer()
...
async def admin_remove_country_selected(query: CallbackQuery, callback_data: CountryCallback, state: FSMContext):
    ...
    try:
        del mock_db["services"][service_name][country_name]
        await state.clear()
        await query.message.edit_text(f"✅ দেশ '{country_name}' ({service_name}) সফলভাবে মুছে ফেলা হয়েছে।")
        logging.info(f"Admin removed country: {country_name} from {service_name}.")
    except KeyError:
        await state.clear(); await query.message.edit_text("❌ ত্রুটি: দেশটি খুঁজে পাওয়া যায়নি।")
    await query.answer()
...
async def handle_num_limit_start(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    await state.set_state(AdminStates.set_num_limit)
    current_limit = mock_db["settings"]["num_limit"]
    await message.answer(
        f"বর্তমান নম্বর লিমিট <b>{current_limit}</b> টি।\n"
        "নতুন লিমিট সংখ্যায় লিখুন (যেমন: 5):",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def handle_num_limit_input(message: Message, state: FSMContext):
    try:
        ...
        if new_limit <= 0:
            await message.answer("লিমিট অবশ্যই 0-এর বেশি হতে হবে।")
            return
        mock_db["settings"]["num_limit"] = new_limit
        await state.clear()
        await message.answer(f"✅ নম্বর লিমিট সফলভাবে <b>{new_limit}</b> টি সেট করা হয়েছে।")
        logging.info(f"Num limit set to {new_limit}")
    except ValueError:
        await message.answer("ত্রুটি: অনুগ্রহ করে শুধু সংখ্যা টাইপ করুন।")
    except Exception as e:
...
async def user_get_number_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "আপনি কোন সার্ভিসের জন্য নম্বর চান?",
        reply_markup=get_services_keyboard(action_prefix="select_for_get")
    )
...
async def user_get_number_service_selected(query: CallbackQuery, callback_data: ServiceCallback, state: FSMContext):
    service_name = callback_data.service_name
    await state.update_data(service_name=service_name)
    await state.set_state(UserStates.get_number_select_country)
    await query.message.edit_text(
        f"সার্ভিস: {service_name}\n\nকোন দেশের নম্বর চান?",
        reply_markup=get_countries_keyboard(service_name, action_prefix="select_for_get")
    )
    await query.answer()
...
async def show_numbers_page(message: Message, state: FSMContext, edit: bool = True):
    ...
    try:
        ...
        builder.row(
            InlineKeyboardButton(text="🌍 Change Country", callback_data=NavCallback(action="change_country").pack()),
        ...
        logging.error(f"Error in show_numbers_page: {e}"); await message.answer(f"একটি ত্রুটি ঘটেছে: {e}")
...
async def handle_change_country(query: CallbackQuery, state: FSMContext):
    ...
    if not service_name:
        await state.clear(); await query.message.edit_text("ত্রুটি। /start দিন।"); return
    await state.set_state(UserStates.get_number_select_country)
    await query.message.edit_text(
        f"সার্ভিস: {service_name}\n\nকোন দেশের নম্বর চান?",
        reply_markup=get_countries_keyboard(service_name, action_prefix="select_for_get")
...
async def handle_support(message: Message, state: FSMContext):
    support_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="👨‍💻 Admin-এর সাথে যোগাযোগ করুন", url=f"t.me/{ADMIN_USERNAME}")]
    ])
    await message.answer(
        "সাপোর্টের জন্য, অনুগ্রহ করে নিচের বাটনে ক্লিক করে অ্যাডমিনের সাথে যোগাযোগ করুন:",
        reply_markup=support_keyboard
    )


# --- ব্যাক বাটন হ্যান্ডলার (Back Button Handlers) ---
...
async def handle_back_button(query: CallbackQuery, callback_data: NavCallback, state: FSMContext):
    ...
    if not current_state_str:
        await query.answer("কিছু করার নেই।", show_alert=True); return
    ...
    if current_state_str in [AdminStates.add_number_input_text.state, AdminStates.add_number_input_file.state]:
        ...
        UserStates.get_number_select_country.state,
        AdminStates.remove_country_select.state
    ]:
        if current_state_str == UserStates.get_number_select_country.state:
            new_state, action = UserStates.get_number_select_service, "select_for_get"
        elif current_state_str == AdminStates.add_country_name.state:
